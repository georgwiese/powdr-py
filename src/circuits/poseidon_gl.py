from powdr import PIL, Expression, FixedColumn, IntermediateColumn, NumberExpression, WitnessColumn, lookup, star
...
PARTIAL_ROUNDS = 22
ROWS_PER_HASH = FULL_ROUNDS + PARTIAL_ROUNDS + 1
...
def dot_product(x: Expression, y: Expression) -> Expression:
    return sum(x_i * y_i for x_i, y_i in zip(x, y))

def poseidon_gl() -> PIL:
    # Fixed columns
    FIRSTBLOCK = FixedColumn("FIRSTBLOCK", star([1] + [0] * (ROWS_PER_HASH - 1)))
    LASTBLOCK = FixedColumn("LASTBLOCK", star([0] * (ROWS_PER_HASH - 1) + [1]))
    LAST = FixedColumn("LAST", star([0] * (ROWS_PER_HASH - 1) + [1]) + [1])
    PARTIAL = FixedColumn("PARTIAL", star(
        [0] * (FULL_ROUNDS // 2) + [1] * PARTIAL_ROUNDS + [0] * (FULL_ROUNDS // 2) + [0]
    ))
    C = [FixedColumn(f"C_{i}", star(constants)) for i, constants in enumerate(ROUND_CONSTANTS)]

    # Witness columns
    state = [WitnessColumn(f"state_{i}") for i in range(STATE_SIZE)]
    output = [WitnessColumn(f"output_{i}") for i in range(OUTPUT_SIZE)]

    # Add round constants, apply S-Box, and multiply with MDS matrix
    # Note that this is more involved, because eStark can only handle degree-2 constraints, so we
    # have to generate intermediate polynomials. This is somethong Powdr could do for us though.
    a = [IntermediateColumn(f"a_{i}", state + C) for i, (state, C) in enumerate(zip(state, C))]
    x2 = [IntermediateColumn(f"x2_{i}", a * a) for i, a in enumerate(a)]
    x4 = [IntermediateColumn(f"x4_{i}", x2 * x2) for i, x2 in enumerate(x2)]
    x6 = [IntermediateColumn(f"x6_{i}", x4 * x2) for i, (x4, x2) in enumerate(zip(x4, x2))]
    x7 = [IntermediateColumn(f"x7_{i}", x6 * a) for i, (x6, a) in enumerate(zip(x6, a))]
    b = [IntermediateColumn(f"b_0", x7[0])] + [IntermediateColumn(f"b_{i}", PARTIAL * (a - x5) + x5)
                                               for i, (a, x5) in enumerate(zip(a, x7)) if i > 0]
    new_state = [IntermediateColumn(f"c_{i}", dot_product(M_row, b)) for i, M_row in enumerate(M)]
    
    # New state should be copied to the state in the next row
    for s, s_new in zip(state, new_state):
        yield (s.n - s_new) * (1-LAST) == 0

    # The output is the first element of the state in the last row
    for o, s in zip(output, state):
        yield LASTBLOCK * (o - s) == 0

    # The output is kept constant within the block
    for o in output:
        yield (o.n - o) * (1 - LAST) == 0
